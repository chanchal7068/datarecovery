...
export default function FAQSection() {
  ...
  return (
    <section className="w-full bg-white pt-24 2xl:pt-28 pb-40 2xl:pb-48 relative overflow-hidden">
      <div className="max-w-[1000px] 2xl:max-w-[1320px] mx-auto px-4 sm:px-6 2xl:px-10 relative z-10">
          ...
          {faqs.map((faq, index) => {
            ...
            return (
              <div key={index} className="flex flex-col rounded-2xl border border-slate-100 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.06)] overflow-hidden">
                ...
                >
                  <span className={`font-bold text-[14px] md:text-[15px] 2xl:text-[17px] ${isOpen ? 'text-[#004b9b]' : 'text-[#0f172a]'}`}>
                    {faq.question}
                  </span>
                ...
                {/* Answer Box */}
                {isOpen && (
                  <div className="px-5 2xl:px-7 pb-5 2xl:pb-6 pt-1 bg-white text-[#64748b] text-[14px] 2xl:text-[16px] leading-relaxed">
                    {faq.answer}
                  </div>
                )}
              </div>
